...
function signToken(payload, expiresInMinutes = 60 * 24 * 7) {
  ...
  const now = Date.now();
  const exp = Math.floor(now / 1000) + (expiresInMinutes * 60);
  const tokenIat = payload.iat || now;
  const data = Buffer.from(JSON.stringify({ ...payload, iat: tokenIat, exp })).toString('base64url');
...
function authenticateRequest(req) {
  ...
  // Admin Session Kill-Switch / Global Invalidation Check
  const isAdmin = ['SUPER_ADMIN', 'ADMIN', 'SUB_ADMIN'].includes(user.role);
  if (isAdmin) {
    const globalRevokedAt = db.settings.adminSessionsRevokedAt || 0;
    const userRevokedAt = user.sessionsRevokedAt || 0;
    const tokenIat = payload.iat || 0;

    // If token was issued before the global or user revocation timestamp, reject immediately
    if (globalRevokedAt > 0 && tokenIat < globalRevokedAt) {
      return null;
    }
    if (userRevokedAt > 0 && tokenIat < userRevokedAt) {
      return null;
    }
  } else {
    if (user.sessionsRevokedAt && payload.iat && payload.iat < user.sessionsRevokedAt) {
      return null;
    }
  }
