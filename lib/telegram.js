...
function generateTelegramLinkToken(userId) {
  ...
  const botUsername = (db.settings.telegramBotUsername || 'freakshowtopupbot').replace(/^@/, '');
...
function getMainMenuKeyboard(isSuperAdmin = true) {
  if (isSuperAdmin) {
    return [
      ...
      [
        { text: '🖼️ Banners & Popups', callback_data: 'nav_banners' },
...
<i>Choose an action to manage customer accounts:</i>
  ...
  const keyboard = [
    [
      ...
      { text: '✏️ Edit User', callback_data: 'usr_edit_prompt' }
    ],
    [
    ...
    [
      { text: '🚫 Ban / Unban', callback_data: 'usr_ban_prompt' },
      { text: '🗑 Delete User', callback_data: 'usr_del_prompt' }
    ],
    [
...
function showUserProfileCard(chatId, messageId, user) {
  ...
  const isVip = Boolean(user.isVip || user.vipUnlocked || user.hasVipAccess);
  const vipBadge = isVip ? '👑 <b>VIP MEMBER (Active)</b>' : 'Standard Member';
...
// ==========================================
// 11.3 BANNERS & POPUPS EMERGENCY CONTROL MODULE
// ==========================================

async function showBannerControlMenu(chatId, messageId = null) {
  const popupActive = db.settings.popupEnabled !== false && (db.settings.offerPopup ? db.settings.offerPopup.isActive !== false : true);
  const heroActive = db.settings.heroBanner ? db.settings.heroBanner.enabled !== false : true;
  const side1Active = db.settings.heroSide1 ? db.settings.heroSide1.enabled !== false : true;
  const side2Active = db.settings.heroSide2 ? db.settings.heroSide2.enabled !== false : true;

  const text = `
🖼️ <b>BANNERS & POPUPS EMERGENCY CONTROL</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
📢 <b>Offer Popup:</b> ${popupActive ? '🟢 <b>ACTIVE (Showing on Site)</b>' : '🔴 <b>DISABLED (Hidden)</b>'}
🖼️ <b>Hero Top Banner:</b> ${heroActive ? '🟢 <b>ACTIVE (Showing)</b>' : '🔴 <b>DISABLED (Hidden)</b>'}
🏷️ <b>Hero Side Card 1:</b> ${side1Active ? '🟢 <b>ACTIVE (Weekly/Monthly)</b>' : '🔴 <b>DISABLED</b>'}
🏷️ <b>Hero Side Card 2:</b> ${side2Active ? '🟢 <b>ACTIVE (Level Up Pass)</b>' : '🔴 <b>DISABLED</b>'}
━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Toggle any banner or popup instantly with 1-click below:</i>
  `.trim();

  const keyboard = [
    [
      { text: popupActive ? '🔴 Turn Popup OFF' : '🟢 Turn Popup ON', callback_data: 'banner_toggle_popup' },
      { text: heroActive ? '🔴 Turn Banner OFF' : '🟢 Turn Banner ON', callback_data: 'banner_toggle_hero' }
    ],
    [
      { text: side1Active ? '🔴 Turn Side 1 OFF' : '🟢 Turn Side 1 ON', callback_data: 'banner_toggle_side1' },
      { text: side2Active ? '🔴 Turn Side 2 OFF' : '🟢 Turn Side 2 ON', callback_data: 'banner_toggle_side2' }
    ],
    [
      { text: '🔄 Refresh Status', callback_data: 'banner_refresh' },
      { text: '⚙️ Settings', callback_data: 'nav_settings' }
    ],
    [
      { text: '🏠 Main Menu', callback_data: 'nav_main' }
...
async function handleTelegramCallbackQuery(callbackQuery) {
  ...
  const isCustomerAction = data.startsWith('usr_games') || data.startsWith('usr_balance') || data.startsWith('usr_deposit') || data.startsWith('usr_orders') || data.startsWith('usr_unlink') || data.startsWith('usr_menu') || data.startsWith('usr_pkg_') || data.startsWith('usr_sub_') || data.startsWith('usr_topup_') || data.startsWith('usr_conf_');
...
<i>Type /cancel to abort.</i>
  ...
  if (data === 'nav_banners' || data === 'banner_refresh') {
    if (!auth.isSuperAdmin) {
      return await answerCallbackQuery(queryId, '⛔ Access Denied! Only Super Admin can manage Banners & Popups.', true);
    }
    return await showBannerControlMenu(chatId, messageId);
  }

  if (data === 'banner_toggle_popup') {
    if (!auth.isSuperAdmin) return await answerCallbackQuery(queryId, '⛔ Super Admin access required.', true);
    const current = db.settings.popupEnabled !== false && (db.settings.offerPopup ? db.settings.offerPopup.isActive !== false : true);
    const next = !current;
    db.settings.popupEnabled = next;
    if (!db.settings.offerPopup) db.settings.offerPopup = {};
    db.settings.offerPopup.isActive = next;
    db.saveAll();
    recordAuditLog({ actorId: `tg_${fromId}`, action: 'POPUP_TOGGLE_TELEGRAM', reason: `Popup set to ${next ? 'ON' : 'OFF'} by ${auth.name}` });
    await answerCallbackQuery(queryId, `Offer Popup is now ${next ? 'ENABLED 🟢' : 'DISABLED 🔴'}`);
    return await showBannerControlMenu(chatId, messageId);
  }

  if (data === 'banner_toggle_hero') {
    if (!auth.isSuperAdmin) return await answerCallbackQuery(queryId, '⛔ Super Admin access required.', true);
    if (!db.settings.heroBanner) db.settings.heroBanner = {};
    const next = db.settings.heroBanner.enabled === false ? true : false;
    db.settings.heroBanner.enabled = next;
    db.saveAll();
    recordAuditLog({ actorId: `tg_${fromId}`, action: 'HERO_BANNER_TOGGLE_TELEGRAM', reason: `Hero banner set to ${next ? 'ON' : 'OFF'} by ${auth.name}` });
    await answerCallbackQuery(queryId, `Hero Banner is now ${next ? 'ENABLED 🟢' : 'DISABLED 🔴'}`);
    return await showBannerControlMenu(chatId, messageId);
  }

  if (data === 'banner_toggle_side1') {
    if (!auth.isSuperAdmin) return await answerCallbackQuery(queryId, '⛔ Super Admin access required.', true);
    if (!db.settings.heroSide1) db.settings.heroSide1 = {};
    const next = db.settings.heroSide1.enabled === false ? true : false;
    db.settings.heroSide1.enabled = next;
    db.saveAll();
    recordAuditLog({ actorId: `tg_${fromId}`, action: 'SIDE1_TOGGLE_TELEGRAM', reason: `Side Card 1 set to ${next ? 'ON' : 'OFF'} by ${auth.name}` });
    await answerCallbackQuery(queryId, `Side Card 1 is now ${next ? 'ENABLED 🟢' : 'DISABLED 🔴'}`);
    return await showBannerControlMenu(chatId, messageId);
  }

  if (data === 'banner_toggle_side2') {
    if (!auth.isSuperAdmin) return await answerCallbackQuery(queryId, '⛔ Super Admin access required.', true);
    if (!db.settings.heroSide2) db.settings.heroSide2 = {};
    const next = db.settings.heroSide2.enabled === false ? true : false;
    db.settings.heroSide2.enabled = next;
    db.saveAll();
    recordAuditLog({ actorId: `tg_${fromId}`, action: 'SIDE2_TOGGLE_TELEGRAM', reason: `Side Card 2 set to ${next ? 'ON' : 'OFF'} by ${auth.name}` });
    await answerCallbackQuery(queryId, `Side Card 2 is now ${next ? 'ENABLED 🟢' : 'DISABLED 🔴'}`);
    return await showBannerControlMenu(chatId, messageId);
  }
  ...
  if (data.startsWith('usr_del_list_p')) {
  ...
  if (data.startsWith('usr_ban_list_p')) {
  ...
  if (data.startsWith('usr_edit_list_p')) {
  ...
  // Delete User Prompt (Asks for Email / TG ID / User ID)
  if (data === 'usr_del_prompt') {
    sessions.setSession(chatId, { step: 'SEARCH_USER', mode: 'usr_del_prompt' });
    const text = `
🗑 <b>DELETE USER ACCOUNT</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
Please type or send the <b>Customer Gmail, Telegram ID, or User ID</b> (e.g. <code>user@gmail.com</code> or <code>1024</code>):
━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Type /cancel to abort.</i>
    `.trim();
    return await sendTelegramMessage(text, chatId, [
      [{ text: '📋 View All Users List', callback_data: 'usr_del_list_p0' }],
      [{ text: '❌ Cancel', callback_data: 'nav_users' }]
    ]);
  }

  // Ban / Unban Prompt (Asks for Email / TG ID / User ID)
  if (data === 'usr_ban_prompt') {
    sessions.setSession(chatId, { step: 'SEARCH_USER', mode: 'usr_ban_prompt' });
    const text = `
🚫 <b>BAN / UNBAN USER ACCESS</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
Please type or send the <b>Customer Gmail, Telegram ID, or User ID</b>:
━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Type /cancel to abort.</i>
    `.trim();
    return await sendTelegramMessage(text, chatId, [
      [{ text: '📋 View All Users List', callback_data: 'usr_ban_list_p0' }],
      [{ text: '❌ Cancel', callback_data: 'nav_users' }]
    ]);
  }

  // Edit User Prompt (Asks for Email / TG ID / User ID)
  if (data === 'usr_edit_prompt') {
    sessions.setSession(chatId, { step: 'SEARCH_USER', mode: 'usr_edit_prompt' });
    const text = `
✏️ <b>EDIT CUSTOMER ACCOUNT</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
Please type or send the <b>Customer Gmail, Telegram ID, or User ID</b>:
━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Type /cancel to abort.</i>
    `.trim();
    return await sendTelegramMessage(text, chatId, [
      [{ text: '📋 View All Users List', callback_data: 'usr_edit_list_p0' }],
      [{ text: '❌ Cancel', callback_data: 'nav_users' }]
    ]);
  }
  ...
  if (data === 'usr_search_prompt') {
    sessions.setSession(chatId, { step: 'SEARCH_USER', mode: 'usr_search_prompt' });
    const text = `
🔎 <b>SEARCH USER</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
Please type the <b>Customer Gmail, Telegram ID, or User ID</b>:
━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Type /cancel to abort.</i>
    `.trim();
    return await sendTelegramMessage(text, chatId, [
      [{ text: '👥 Show All Users', callback_data: 'usr_list_p0' }],
      [{ text: '❌ Cancel', callback_data: 'nav_users' }]
    ]);
  }
...
<i>You will now receive automatic order delivery notifications & cash refund alerts right here!</i>
  ...
  if (userShopSession && userShopSession.step === 'AWAITING_UID') {
    ...
    const cancelKeywords = ['/cancel', 'cancel', 'no', 'না', 'ভুল', 'exit', 'stop', 'abort', 'cancle', 'বাতিল', 'na', 'dorkar nai', 'lagbe na', 'lagbena', 'cencel'];
    if (cancelKeywords.includes(text.toLowerCase().trim())) {
      userShoppingSessions.delete(String(chatId));
      return await sendTelegramMessage('🚫 <b>Order Cancelled Safely</b>\nYour pending top-up order has been cancelled. No funds were deducted.', chatId, [
...
<i>Sent by: ${auth.name || 'Master Admin'}</i>
  ...
  // Customer VIP Access Unlock Command: /vip <CODE>
  if (text.toLowerCase().startsWith('/vip ') || text.toLowerCase().startsWith('/vipcode ')) {
    const code = text.split(' ').slice(1).join(' ').trim().toUpperCase();
    let user = db.users.find(u => 
      (u.telegramChatId && (String(u.telegramChatId) === String(chatId) || String(u.telegramChatId) === String(fromId))) ||
      (u.telegramId && (String(u.telegramId) === String(chatId) || String(u.telegramId) === String(fromId)))
    );

    if (!user) {
      // Auto-create customer profile on the fly for Telegram user
      const newId = `usr_tg_${chatId}`;
      user = {
        id: newId,
        name: (auth && auth.name) || 'Telegram User',
        email: `tg_${chatId}@telegram.local`,
        role: 'CUSTOMER',
        telegramChatId: String(chatId),
        telegramId: String(chatId),
        telegramUsername: auth && auth.username ? `@${auth.username}` : '',
        currency: 'BDT',
        status: 'ACTIVE',
        createdAt: new Date().toISOString()
      };
      db.users.push(user);
      if (!db.wallets.some(w => w.userId === newId)) {
        db.wallets.push({
          id: `w_${newId}`,
          userId: newId,
          balance: 0,
    ...
    if (!code) {
      return await sendTelegramMessage('⚠️ Usage: <code>/vip <CODE></code> (Example: <code>/vip JOY100LVL</code>)', chatId);
    }

    let isValid = false;
    if (db.settings.vipAccessCodeHash) {
      isValid = await bcrypt.compare(code, db.settings.vipAccessCodeHash);
    } else if (db.settings.currentVipCode || db.settings.vipAccessCode) {
      isValid = code === String(db.settings.currentVipCode || db.settings.vipAccessCode).trim().toUpperCase();
    } else {
      isValid = code === 'JOY100LVL';
    }
    ...
    if (isValid) {
      ...
      user.vipUnlockedAt = new Date().toISOString();
      user.vipCodeVersion = db.settings.vipCodeVersion || 1;
      db.saveAll();

      return await sendTelegramMessage(`
🎉 <b>VIP ACCESS UNLOCKED!</b> 👑
━━━━━━━━━━━━━━━━━━━━━━━━━━
Congratulations <b>${user.name || 'Gamer'}</b>, your VIP status is now <b>ACTIVE</b>!
You can now enjoy special discounted prices on VIP Weekly, Monthly, and Level Up Passes.
━━━━━━━━━━━━━━━━━━━━━━━━━━
      `.trim(), chatId, [
        [{ text: '👑 Browse VIP Packages', callback_data: 'usr_sub_sub-ff-vip' }],
        [{ text: '🏠 Main Menu', callback_data: 'usr_menu' }]
      ]);
    } else {
      return await sendTelegramMessage('⚠️ <b>Invalid VIP Code!</b> Please check your code and try again.', chatId);
    }
  }
  ...
  // Emergency Banner & Popup Controls via Slash Commands
  if (lowerText === '/banner' || lowerText === '/banners' || lowerText === '/popup' || lowerText === '/popups') {
    if (!auth.isSuperAdmin) return await sendTelegramMessage('⛔ Super Admin access required.', chatId);
    return await showBannerControlMenu(chatId);
  }

  if (lowerText === '/popup on' || lowerText === '/popup enable' || lowerText === '/popup 1') {
    if (!auth.isSuperAdmin) return await sendTelegramMessage('⛔ Super Admin access required.', chatId);
    db.settings.popupEnabled = true;
    if (!db.settings.offerPopup) db.settings.offerPopup = {};
    db.settings.offerPopup.isActive = true;
    db.saveAll();
    recordAuditLog({ actorId: `tg_${fromId}`, action: 'POPUP_ENABLE_TELEGRAM', reason: `Offer popup turned ON by ${auth.name}` });
    return await sendTelegramMessage('📢 <b>Offer Popup is now ENABLED 🟢</b>\nIt will now show on website for all visitors.', chatId, [
      [{ text: '🖼️ Banner & Popup Hub', callback_data: 'nav_banners' }]
    ]);
  }

  if (lowerText === '/popup off' || lowerText === '/popup disable' || lowerText === '/popup 0') {
    if (!auth.isSuperAdmin) return await sendTelegramMessage('⛔ Super Admin access required.', chatId);
    db.settings.popupEnabled = false;
    if (!db.settings.offerPopup) db.settings.offerPopup = {};
    db.settings.offerPopup.isActive = false;
    db.saveAll();
    recordAuditLog({ actorId: `tg_${fromId}`, action: 'POPUP_DISABLE_TELEGRAM', reason: `Offer popup turned OFF by ${auth.name}` });
    return await sendTelegramMessage('📢 <b>Offer Popup is now DISABLED 🔴</b>\nIt is now hidden on website.', chatId, [
      [{ text: '🖼️ Banner & Popup Hub', callback_data: 'nav_banners' }]
    ]);
  }

  if (lowerText === '/banner on' || lowerText === '/banner enable' || lowerText === '/banner 1') {
    if (!auth.isSuperAdmin) return await sendTelegramMessage('⛔ Super Admin access required.', chatId);
    if (!db.settings.heroBanner) db.settings.heroBanner = {};
    db.settings.heroBanner.enabled = true;
    db.saveAll();
    recordAuditLog({ actorId: `tg_${fromId}`, action: 'BANNER_ENABLE_TELEGRAM', reason: `Hero banner turned ON by ${auth.name}` });
    return await sendTelegramMessage('🖼️ <b>Hero Top Banner is now ENABLED 🟢</b>', chatId, [
      [{ text: '🖼️ Banner & Popup Hub', callback_data: 'nav_banners' }]
    ]);
  }

  if (lowerText === '/banner off' || lowerText === '/banner disable' || lowerText === '/banner 0') {
    if (!auth.isSuperAdmin) return await sendTelegramMessage('⛔ Super Admin access required.', chatId);
    if (!db.settings.heroBanner) db.settings.heroBanner = {};
    db.settings.heroBanner.enabled = false;
    db.saveAll();
    recordAuditLog({ actorId: `tg_${fromId}`, action: 'BANNER_DISABLE_TELEGRAM', reason: `Hero banner turned OFF by ${auth.name}` });
    return await sendTelegramMessage('🖼️ <b>Hero Top Banner is now DISABLED 🔴</b>', chatId, [
      [{ text: '🖼️ Banner & Popup Hub', callback_data: 'nav_banners' }]
    ]);
  }
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  // 1. Search User Step (Prompt receiver for Delete, Ban, Edit, and Search)
  if (session.step === 'SEARCH_USER' || session.step === 'SEARCH_USER_BALANCE') {
    const searchMode = session.mode;
    sessions.clearSession(chatId);
    const query = text.toLowerCase().replace('#', '').trim();
    const cleanTgQuery = query.replace(/^@/, '');

    const user = (db.users || []).find(u => 
      (u.id && u.id.toLowerCase() === query) || 
      (u.email && u.email.toLowerCase() === query) ||
      (u.username && u.username.toLowerCase() === cleanTgQuery) ||
      (u.name && u.name.toLowerCase() === query) ||
      (u.telegramChatId && String(u.telegramChatId) === query) ||
      (u.telegramId && String(u.telegramId) === query)
    );

    if (!user) {
      return await sendTelegramMessage(`❌ User not found for: <code>${text}</code>\n<i>Please check the Gmail, Telegram ID, or User ID and try again.</i>`, chatId, [
        [{ text: '🔄 Try Again', callback_data: searchMode || 'usr_search_prompt' }, { text: '👥 Users Menu', callback_data: 'nav_users' }]
      ]);
    }

    if (searchMode === 'usr_del_prompt') {
      const textDel = `
⚠️ <b>CONFIRM USER DELETION</b> ⚠️
━━━━━━━━━━━━━━━━━━━━━━━━━━
Found User:
...
📧 <b>Email:</b> <code>${user.email}</code>
🆔 <b>User ID:</b> <code>#${user.id}</code>
💰 <b>Balance:</b> ৳${((db.wallets.find(w => w.userId === user.id) || {}).balance || 0)} BDT
━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Are you sure you want to permanently delete this user?</i>
      `.trim();

      return await sendTelegramMessage(textDel, chatId, [
        [
          { text: '🗑 Yes, Delete User', callback_data: `usr_deleteexec_${user.id}` },
          { text: '❌ Cancel', callback_data: 'nav_users' }
        ]
    ...
    if (searchMode === 'usr_ban_prompt') {
      ...
      recordAuditLog({
        ...
        before: { status: oldStatus },
        after: { status: newStatus },
        reason: `Status changed via search prompt by ${auth.name}`
      });

      const banMsg = `
${newStatus === 'BANNED' ? '🚫' : '✅'} <b>USER STATUS UPDATED: ${newStatus}</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
👤 <b>User:</b> ${user.name || user.username}
📧 <b>Email:</b> <code>${user.email}</code>
🆔 <b>ID:</b> <code>#${user.id}</code>
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
      ...
      return await sendTelegramMessage(banMsg, chatId, [
        [{ text: '👤 View Profile', callback_data: `usr_search_exec_${user.id}` }, { text: '👥 Users Menu', callback_data: 'nav_users' }]
      ]);
    }
    ...
    if (session.step === 'SEARCH_USER_BALANCE') {
      return await handleTelegramCallbackQuery({
        id: 'direct_call',
        from: { id: fromId, first_name: auth.name },
...
async function registerBotMenuAndCommands(token) {
  try {
    // 1. Clean Customer Commands for all public users
    const customerCommands = [
      { command: 'start', description: '🎮 Open Main Menu / Dashboard' },
      { command: 'topup', description: '🎯 Game Top-Up & Diamond Recharge' },
      { command: 'balance', description: '💰 Check Wallet Balance' },
      { command: 'deposit', description: '➕ Deposit Money to Wallet' },
      { command: 'orders', description: '📦 My Order History' },
      { command: 'vip', description: '👑 Unlock VIP Pass (/vip CODE)' },
    ...
    // 2. Admin Management Commands for Master Admin (5339688506)
    const adminCommands = [
      { command: 'start', description: '👑 Admin Control Hub' },
      ...
      { command: 'refrate', description: '💸 Set Referral % (/refrate 2.5)' },
      { command: 'usdrate', description: '💱 Set 1 USD Rate (/usdrate 125)' },
      { command: 'popup', description: '🖼️ Toggle Offer Popup (/popup on|off)' },
      { command: 'banner', description: '🖼️ Toggle Hero Banner (/banner on|off)' },
      { command: 'settings', description: '⚙️ Payment & Site Settings' },
      { command: 'stats', description: '📊 Live Sales Analytics' },
      { command: 'broadcast_users', description: '📢 Send Broadcast Message' },
      { command: 'cancel', description: '❌ Cancel Current Action' }
    ];

    await sendTelegramRequest('setMyCommands', {
      commands: adminCommands,
      scope: { type: 'chat', chat_id: 5339688506 }
    });

    await sendTelegramRequest('setChatMenuButton', {
      menu_button: { type: 'commands' }
    });
    console.log('🤖 [Telegram Bot] Separate Customer & Admin Commands Menus registered.');
  } catch (e) {
...
async function showUserMenu(chatId, messageId = null, user = null) {
  ...
  if (!targetUser) {
    const welcomeMsg = `
👋 <b>Welcome to FREAK SHOW Bot!</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
⚡ <i>Instant Automated Free Fire Diamonds & Game Top-Up.</i>

🔗 <b>Connect your Account:</b>
Login to our website and click <b>"Connect Telegram"</b> in your profile to sync your balance & orders!
━━━━━━━━━━━━━━━━━━━━━━━━━━
    `.trim();

    const buttons = [
      [{ text: '🎯 Browse Games', callback_data: 'usr_games' }, { text: '🌐 Website Shop', url: siteUrl }]
    ];
  ...
  const isVip = Boolean(targetUser.isVip || targetUser.vipUnlocked || targetUser.hasVipAccess);
  const vipBadge = isVip ? '👑 <b>VIP Status:</b> <b>👑 VIP MEMBER (Active)</b>\n' : '';

  const userMsg = `
🎮 <b>FREAK SHOW GAMER HUB</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
👤 <b>Account:</b> <b>${targetUser.name || 'Gamer'}</b>
${vipBadge}💰 <b>Wallet Balance:</b> <b>৳${balance} ${currency}</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Choose an option below:</i>
  ...
  const buttons = [
    ...
    [{ text: '➕ Deposit Money', callback_data: 'usr_deposit' }, { text: '📦 My Orders', callback_data: 'usr_orders' }],
    [{ text: '🌐 Website Shop', url: siteUrl }]
  ];
...
async function showUserGames(chatId, messageId) {
  const categories = [
    ...
    { id: 'sub-ff-vip', name: '👑 VIP Access Pass' },
...
async function showUserCategoryPackages(chatId, messageId, subcategoryId) {
  const user = db.users.find(u => 
    (u.telegramChatId && (String(u.telegramChatId) === String(chatId) || String(u.telegramChatId) === String(messageId))) ||
    (u.telegramId && (String(u.telegramId) === String(chatId) || String(u.telegramId) === String(messageId)))
  );
  const isVip = Boolean(user && (user.isVip || user.vipUnlocked || user.hasVipAccess));

  let products = [];
  if (subcategoryId === 'sub-ff-vip') {
    if (!isVip) {
      const vipPrompt = `
👑 <b>VIP ACCESS PASS REQUIRED</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
🔒 <i>This VIP category contains exclusive discounted passes for active VIP members only!</i>

🔑 <b>Unlock VIP Access:</b>
Send <code>/vip <CODE></code> here in chat (e.g. <code>/vip JOY100LVL</code>) or unlock it on our website to access these discounted rates!
━━━━━━━━━━━━━━━━━━━━━━━━━━
      `.trim();
      return await editTelegramMessage(chatId, messageId, vipPrompt, [
        [{ text: '🌐 Website Shop', url: 'https://' + (db.settings.domain || 'freakshowtopup.shop') }],
        [{ text: '« Back to Games', callback_data: 'usr_games' }]
      ]);
    }
    products = db.products.filter(p => (p.subcategoryId === 'sub-ff-vip' || p.isVip) && p.isActive !== false)
...
async function notifyUserDeposit(deposit, newBalance) {
  ...
  if (!user) return;
  const targetChatId = user.telegramChatId || user.telegramId;
...
💳 <b>Method:</b> <b>${deposit.paymentMethod || deposit.method ? String(deposit.paymentMethod || deposit.method).toUpperCase() : 'MANUAL'}</b>
💼 <b>New Wallet Balance:</b> <b>৳${Number(newBalance).toFixed(2)} BDT</b>
🕒 <b>Time:</b> ${new Date().toLocaleTimeString()}
━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Your balance is now updated. You can order instant game top-ups right now!</i>
  `.trim();

  return await sendTelegramMessage(msg, targetChatId, [
    [{ text: '🎯 Game Top-Up', callback_data: 'usr_games' }, { text: '💰 Check Balance', callback_data: 'usr_balance' }],
    [{ text: '🛍️ Website Shop', url: 'https://' + (db.settings.domain || 'freakshowtopup.shop') }]
  ]);
}
