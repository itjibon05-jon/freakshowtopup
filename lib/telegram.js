...
async function renderUsersListPage(chatId, messageId, page = 0) {
  ...
  const pageUsers = db.users.slice(startIdx, startIdx + pageSize);
...
<i>Type /cancel to abort.</i>
  ...
  // Search User Prompt
  if (data === 'usr_search_prompt' || data === 'usr_edit_prompt' || data === 'usr_ban_prompt' || data === 'usr_del_prompt') {
...
<i>Type /cancel to abort.</i>
  ...
  if (data.startsWith('usr_delconfirm_')) {
    ...
    const text = `
⚠️ <b>PERMANENT SAFETY WARNING</b> ⚠️
━━━━━━━━━━━━━━━━━━━━━━━━━━
Are you sure you want to <b>DELETE / SOFT-DELETE</b> this user?
👤 <b>User:</b> ${user.name || user.username}
📧 <b>Email:</b> <code>${user.email}</code>
🆔 <b>ID:</b> <code>${user.id}</code>
━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Financial ledger history will be preserved for accounting.</i>
    `.trim();

    const keyboard = [
      [
        { text: '🗑 Yes, Delete User', callback_data: `usr_deleteexec_${user.id}` },
        { text: '❌ Cancel', callback_data: `usr_search_exec_${user.id}` }
      ]
    ];
    return await editTelegramMessage(chatId, messageId, text, keyboard);
  }
  ...
  if (data.startsWith('usr_deleteexec_')) {
    ...
    if (idx !== -1) {
      ...
      deletedUser.email = `deleted_${Date.now()}_${deletedUser.email}`;
      db.saveAll();
      ...
      await answerCallbackQuery(queryId, 'User account deleted successfully.', true);
...
<i>Sent by: ${auth.name || 'Master Admin'}</i>
  ...
  if (lowerText.startsWith('/vipcode ') || lowerText.startsWith('/setvip ') || lowerText === '/vipreset') {
    ...
    let newCode = text.split(' ').slice(1).join(' ').trim().toUpperCase();
    if (!newCode) {
...
🔄 <b>Old VIP Access:</b> <b>REVOKED 🚫</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  if (lowerText === '/admins' || lowerText === '/admin' || lowerText.includes('admin')) return await showAdminsMenu(chatId);
  if (lowerText === '/stats' || lowerText === '/statistics' || lowerText.includes('stat')) return await showStatsMenu(chatId);
  if (lowerText === '/settings' || lowerText === '/setting' || lowerText.includes('setting')) return await showSettingsMenu(chatId);
