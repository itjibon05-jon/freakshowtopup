...
<i>Sent by: ${auth.name || 'Master Admin'}</i>
  ...
  if (text.toLowerCase().startsWith('/vip ') || text.toLowerCase().startsWith('/vipcode ')) {
    ...
    const user = db.users.find(u => u.telegramChatId && (String(u.telegramChatId) === String(chatId) || String(u.telegramChatId) === String(fromId)));
    if (!user) {
      return await sendTelegramMessage('⚠️ Please connect your Telegram account first by typing /start or linking from website.', chatId);
    }
    if (!code) {
...
async function showUserCategoryPackages(chatId, messageId, subcategoryId) {
  const user = db.users.find(u => u.telegramChatId && (String(u.telegramChatId) === String(chatId) || String(u.telegramChatId) === String(messageId)));
